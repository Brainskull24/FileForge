export type ToolType = "text" | "file"
...
export const toolConfigs: Record<string, BaseToolConfig> = {
  ...
  "qr-to-text": {
    ...
    description: "Extract text from QR code images",
    type: "file",
    category: "encoding",
    acceptFiles: true,
    fileTypes: ["image/*"],
    placeholder: "Upload QR code images to extract text",
  },
  "barcode-to-text": {
    name: "Barcode to Text",
    description: "Extract text from barcode images",
    type: "file",
    category: "encoding",
    acceptFiles: true,
    fileTypes: ["image/*"],
    placeholder: "Upload barcode images to extract text",
  },
  ...
  "image-conversion": {
    ...
    formats: {
      supported: ["JPG", "PNG", "WebP", "GIF", "BMP", "TIFF", "SVG", "HEIC", "ICO", "AVIF", "JPEG2000"],
    },
  },
  "audio-conversion": {
    ...
    formats: {
      supported: ["MP3", "WAV", "FLAC", "AAC", "OGG", "M4A", "WMA", "AIFF", "OPUS", "WebM"],
    },
  },
  "video-conversion": {
    ...
    formats: {
      supported: ["MP4", "AVI", "MOV", "WebM", "MKV", "FLV", "WMV", "M4V", "3GP", "MPEG"],
    },
  },
  "archive-conversion": {
    name: "Archive Files",
  ...
  "text-conversion": {
    ...
    formats: {
      supported: ["TXT", "CSV", "JSON", "XML", "HTML", "MD", "RTF", "CSS", "JS", "YAML", "SQL"],
    },
  },
...
export function getToolsByCategory(category?: string): Record<string, BaseToolConfig> {
  if (!category) return toolConfigs

  return Object.fromEntries(Object.entries(toolConfigs).filter(([_, config]) => config.category === category))
}

// Helper function to get tools by type
export function getToolsByType(type: ToolType): Record<string, BaseToolConfig> {
  return Object.fromEntries(Object.entries(toolConfigs).filter(([_, config]) => config.type === type))
}
