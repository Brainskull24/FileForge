...
const VerificationFailed: React.FC = () => {
  ...
  const handleResendVerification = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message = error?.response?.data?.message || error?.response?.data?.error || "Failed to resend email"
      alert(message)
    } finally {
      setLoading(false)
    }
      ...
      <Card className="relative max-w-md w-full bg-white/70 backdrop-blur-xl shadow-2xl border border-white/20">
        <CardContent className="relative p-8">
          ...
          <div className="text-center mt-8">
            ...
            {/* Footer */}
            <div className="mt-8">
              <p className="text-xs text-slate-500">Still having trouble? Contact our support team for assistance.</p>
            </div>
          </div>
        </CardContent>
      </Card>
