...
export function UniversalConverterDashboard() {
  ...
  const handleFileProcess = (files: File[], operation: string) => {
    ...
    setTimeout(() => {
      setProcessingJobs((prev) =>
        prev.map((job) =>
          newJobs.find((newJob) => newJob.id === job.id) ? { ...job, status: "processing" as const } : job,
        ),
      )
    }, 500)
  ...
  return (
    <SidebarProvider defaultOpen={true}>
      <div className="flex h-screen w-full bg-background">
        <AppSidebar selectedTool={selectedTool} onToolSelect={handleToolSelect} />

        <div className="flex flex-1 flex-col overflow-hidden">
          <DashboardHeader userCredits={userCredits} />

          <div className="flex flex-1 overflow-hidden">
            <MainWorkspace selectedTool={selectedTool} onFileProcess={handleFileProcess} userCredits={userCredits} />

            <ResultsPanel processingJobs={processingJobs} userCredits={userCredits} />
          </div>
        </div>
      </div>
    </SidebarProvider>
