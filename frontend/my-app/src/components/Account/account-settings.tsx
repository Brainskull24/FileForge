...
import { ProfileTab } from "./account-settings/profile-tab"
import { SecurityTab } from "./account-settings/security-tab"
import { PreferencesTab } from "./account-settings/preferences-tab"
import { BillingTab } from "./account-settings/billing-tab"
import { ApiKeysTab } from "./account-settings/api-keys-tab"
import { PrivacyTab } from "./account-settings/privacy-tab"
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <Badge variant="secondary" className="flex items-center gap-2">
          ...
          All changes saved
          <span className="text-xs text-muted-foreground ml-1">{lastSaved.toLocaleTimeString()}</span>
        </Badge>
      </div>
          ...
          <div className="hidden lg:block lg:w-64">
            <div className="space-y-1">
              ...
              <button
                ...
                className={`w-full text-left px-4 py-2 rounded-md text-sm transition-colors ${
                  activeTab === "preferences" ? "bg-primary text-primary-foreground" : "hover:bg-muted"
                }`}
              >
                Preferences
              </button>
              <button
                ...
                className={`w-full text-left px-4 py-2 rounded-md text-sm transition-colors ${
                  activeTab === "billing" ? "bg-primary text-primary-foreground" : "hover:bg-muted"
                }`}
              >
                Billing
              </button>
